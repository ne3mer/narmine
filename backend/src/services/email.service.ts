...
type EmailServiceType = 'resend' | 'console';
...
class EmailService {
  private serviceType: EmailServiceType;

  constructor() {
    // Use Resend if API key is available, otherwise fallback to console
    this.serviceType = env.RESEND_API_KEY ? 'resend' : 'console';
  ...
  private async logToConsole(options: EmailOptions) {
