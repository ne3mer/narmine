...
exports.notifyAdminsOfEvent = void 0;
const env_1 = require("../config/env");
const email_service_1 = require("./email.service");
const user_model_1 = require("../models/user.model");
const emailPlaceholders = () => env_1.env.ADMIN_NOTIFICATION_EMAILS
    ? env_1.env.ADMIN_NOTIFICATION_EMAILS.split(',')
    ...
    switch (event.type) {
        case 'order_created': {
            ...
            <li>
              ...
              <span style="color:#6b7280;"> — ${item.quantity} عدد ${typeof item.price === 'number'
          ...
          `)
            ...
            const noteSection = event.note
                ? section('توضیحات مشتری', `<div style="padding:12px;border-radius:16px;background:#fff7ed;border:1px solid #ffedd5;color:#9a3412;">${formatMultiline(event.note)}</div>`)
                : '';
            const body = `
        ${section('سفارش جدید در سیستم ثبت شد', '<p style="margin:0;color:#374151;">لطفاً سفارش جدید را در پنل مدیریت بررسی کنید.</p>')}
        ${section('جزئیات سفارش', summary)}
        ${noteSection}
        ${section('اقلام سفارش', `<ul style="padding-right:18px;margin:0;color:#111827;">${itemsHtml}</ul>`)}
      `;
      ...
      `;
        ...
        case 'review_submitted': {
            ...
            const reviewerInfo = event.submittedBy
                ...
                : '';
            const body = `
        ${section('یک پیام/نظر جدید ثبت شد', '<p style="margin:0;color:#374151;">کاربر جدیدی برای یکی از بازی‌ها نظر ارسال کرده است.</p>')}
        ${section('جزئیات پیام', renderKeyValueList([
                ['بازی', escapeHtml(event.gameTitle)],
                ['امتیاز', `${event.rating} / 5`],
                ['شناسه نظر', escapeHtml(event.reviewId)],
                ['تاریخ', formatDate(event.createdAt)]
            ]))}
        ${section('متن پیام', `<div style="padding:12px;border-radius:16px;background:#ecfeff;border:1px solid #a5f3fc;color:#0f172a;">${formatMultiline(event.comment)}</div>`)}
        ${reviewerInfo ? section('ارسال‌کننده', reviewerInfo) : ''}
      `;
            return { subject, html: wrapEmail('پیام جدید کاربران', body) };
        }
        default: {
            const metaSection = event.type === 'custom' && event.meta
                ? section('اطلاعات تکمیلی', renderKeyValueList(Object.entries(event.meta).map(([key, value]) => [
                    key,
                    escapeHtml(String(value ?? '---'))
                ])))
                : '';
            const subject = event.type === 'custom'
                ? event.subject
                : 'اعلان جدید برای مدیران';
            const message = event.type === 'custom'
                ? event.message
                : 'رویداد جدیدی در سیستم ثبت شد.';
            const body = `
        ${section('رویداد جدید', `<p style="margin:0;color:#374151;">${formatMultiline(message)}</p>`)}
        ${metaSection}
      `;
...
const notifyAdminsOfEvent = async (event) => {
    try {
...
exports.notifyAdminsOfEvent = notifyAdminsOfEvent;
